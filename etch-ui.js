...
document.addEventListener("pointerdown", function(event){
    if(event.buttons === 1){ startDraw(event)}
});
...
SIDENAV.appendChild(COLORPICKER)
...
SIDENAV.append(increasePixel, decreasePixel,showsize)
...
decreasePixel.addEventListener("click",() =>{
    ...
    showsize.textContent =" Size: "+x
    if(x < 1 || y < 1){
        return x=0.5, y=0.5, showsize.textContent =" Size: "+x
    }
})
...
const GRID = document.createElement("optgroup");
GRID.setAttribute("label","Grid")
...
GRID.append(NOGRIDOPT,GRID16X16,GRID32X32,GRID64X64)
